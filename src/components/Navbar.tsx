      ...
      {isMobileMenuOpen && (
        <div className="md:hidden" id="mobile-menu">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-neutral-50 border-t border-neutral-200">
            <Link
              href="/post-a-job"
              className="bg-secondary text-white block px-3 py-2 rounded-md text-base font-medium text-center mb-2"
            >
              Post a Job
            </Link>
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`${getLinkClassName(link.href)} block`}
              >
                {' '}
                {/* block makes it take full width */}
                {link.label}
              </Link>
            ))}
