...
import { getJobsByTagAndLocation } from '@/lib/firestoreClient';
...
const skillLocationScrollConfig = (skill: string, location: string): ScrollRestorationConfig => ({
    listKey: `skillLocationJobs-${skill}-${location}`,
    lastDocIdKey: `skillLocationJobsLastDoc-${skill}-${location}`,
    hasMoreKey: `skillLocationJobsHasMore-${skill}-${location}`,
    scrollPosKey: `skillLocationJobsScrollPos-${skill}-${location}`,
    pageKey: `skillLocationJobsPage-${skill}-${location}`,
    pageCursorsKey: `skillLocationJobsPageCursors-${skill}-${location}`,
    filtersKey: `skillLocationJobsFilters-${skill}-${location}`,
});

const SkillLocationPage: NextPage<SkillLocationPageProps> = ({
  skill,
  location,
  initialJobs,
  lastJobDocId: staticLastJobDocId,
}) => {
    const config = skillLocationScrollConfig(skill, location);
  ...
  const fetchJobs = useCallback(async (cursor: string | null) => {
    ...
    try {
      const response = await fetch(
        `/api/jobs/search?location=${encodeURIComponent(location)}&tags=${encodeURIComponent(skill)}&startAfter=${cursor || ''}&limit=10`
      );
  ...
  return (
    <Layout>
      <Head>
        <title>{`${capitalize(skill)} Jobs in ${location} | AI Job Spot`}</title>
        <meta
          name="description"
          content={`Find the latest ${skill} jobs in ${location}. Browse top AI opportunities.`}
        />
      </Head>
      <div className="max-w-6xl mx-auto px-4 py-12 text-center">
        <h1 className="page-title text-4xl sm:text-5xl md:text-6xl mb-4">
          {capitalize(skill)} Jobs in {location}
        </h1>
        <p className="mt-2 text-lg text-neutral-600 font-sans max-w-2xl mx-auto">
          Explore the latest {skill} opportunities in {location}.
        </p>
        <div className="mt-12 text-left">
          {displayedJobs.length > 0 ? (
            ...
            <div className="text-center py-16 px-6 bg-neutral-50/70 rounded-lg border border-neutral-200/80">
              ...
              <p className="text-neutral-600 mt-2 max-w-md mx-auto">There are currently no {skill} positions in {location}. Please check back later.</p>
            </div>
          )}
...
export const getStaticProps: GetStaticProps<
  SkillLocationPageProps,
  { skill: string; location: string }
> = async (context) => {
  const skill = context.params?.skill;
  ...
  const { jobs, lastVisible: lastJobDoc } = await getJobsByTagAndLocation(decodedSkill, decodedLocation, 10);
