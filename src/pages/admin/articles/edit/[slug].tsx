...
const EditArticlePage: React.FC<EditArticleProps> = ({ article }) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.slug) {
      ...
      newErrors.slug = 'URL Slug must be lowercase, alphanumeric, and use hyphens for spaces (e.g., my-awesome-article).';
    }
    if (!formData.publishDate) newErrors.publishDate = 'Publish Date is required.';
    if (!formData.contentBody || formData.contentBody === '<p><br></p>') newErrors.contentBody = 'Article Content is required.';

    if (formData.issueNo !== undefined && (isNaN(formData.issueNo) || formData.issueNo <= 0)) {
      newErrors.issueNo = 'Issue Number must be a positive number.';
    }
    if (formData.volumeNo !== undefined && (isNaN(formData.volumeNo) || formData.volumeNo <= 0)) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="border-b border-neutral-300 pb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input type="text" id="slug" name="slug" value={formData.slug || ''} onChange={handleChange} className={`w-full p-3 rounded-md border ${errors.slug ? 'border-red-500' : 'border-neutral-300'} outline-none transition`} required />
          ...
          <div className="bg-neutral-50 p-8 rounded-xl shadow-lg border border-neutral-200">
            ...
            <div className="prose lg:prose-xl max-w-none">
              <h1 className="text-4xl font-serif font-bold text-primary-dark mb-4">{formData.title || 'Article Title Goes Here'}</h1>
              <div className="text-sm text-neutral-600 mb-6">
                <span>By {formData.author || 'Author Name'}</span>
                <span className="mx-2">|</span>
                <span>Published on: {formData.publishDate ? new Date(formData.publishDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : 'Select a date'}</span>
              </div>
              <div
                className="article-content"
                dangerouslySetInnerHTML={{ __html: formData.contentBody || '<p>Your article content will appear here...</p>' }}
              />
            </div>
          </div>
        </div>
...
export const getServerSideProps: GetServerSideProps<EditArticleProps> = async (context) => {
  ...
  try {
    ...
    const serializedArticle = {
      ...rest,
      publishDate: publishDate ? (('toDate' in publishDate) ? (publishDate as { toDate: () => Date }).toDate().toISOString() : new Date(publishDate).toISOString()) : null,
    };
