...
function astNodeToObject(node) {
    ...
    if (ts.isCallExpression(node) && node.expression.getText().includes('Timestamp.fromDate')) {
        if (node.arguments.length > 0 && ts.isNewExpression(node.arguments[0])) {
            const newExp = node.arguments[0];
            if (newExp.arguments.length > 0 && ts.isStringLiteral(newExp.arguments[0])) {
...
function createYamlFrontmatter(metadata, fieldsToExclude) {
    ...
    for (const [key, value] of Object.entries(metadata)) {
        ...
        if (value === null || value === undefined) {
            ...
            lines.push(`${key}:`);
            value.forEach(item => lines.push(`  - "${String(item).replaceAll('"', '\"')}"`));
        } else {
...
function main() {
    console.log(`Reading and parsing ${path.basename(SEED_FILE_PATH)} with the TypeScript Compiler API...`);
    const sourceCode = fs.readFileSync(SEED_FILE_PATH, 'utf8');
    const sourceFile = ts.createSourceFile(SEED_FILE_PATH, sourceCode, ts.ScriptTarget.Latest, true);
    ...
    if (!articlesData || !jobsData) {
        throw new Error(`Failed to find 'articles' or 'jobs' array nodes in the AST.`);
    }

    console.log(`Successfully parsed ${articlesData.length} articles and ${jobsData.length} jobs.`);
    ...
    for (const job of jobsData) {
        ...
        if (fs.existsSync(filePath)) {
            const mdContent = fs.readFileSync(filePath, 'utf8');
            if (!mdContent.trim().startsWith('---')) {
                const frontmatter = createYamlFrontmatter(job, ['description', 'responsibilities', 'qualifications', 'preferredQualifications']);
                fs.writeFileSync(filePath, frontmatter + mdContent, 'utf8');
                console.log(`Migrated: ${markdownFile}`);
            } else {
                console.log(`Skipped (frontmatter exists): ${markdownFile}`);
