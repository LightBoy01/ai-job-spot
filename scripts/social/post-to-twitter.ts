...
async function getUnpostedJob(): Promise<Job | null> {
  ...
  for (const doc of snapshot.docs) {
    ...
    // Quality Gate: Must not have been posted, must have a salary, and must have a tweetable description.
    if (!jobData.socialsPosted?.twitter && jobData.salaryRange && jobData.tweetableDescription) {
...
function formatTweetThread(job: Job): string[] {
  ...
  // Strategic Hashtags: Always include #AI and #Jobs, then add job-specific tags.
...
async function main() {
  ...
  try {
    ...
    if (isDryRun) {
      ...
      console.log('Would NOT attach an image.');
